...
پیش‌فرض، روی PRهای باز و روی الگوی provider اجرا می‌شود. می‌شود دستههای دیگر (custom یا generic) را هم به بلاک اضافه کرد. کسی که bypass permission ندارد باید تک‌تک alert را ببندد تا بلاک برداشته شود.
...
## با push protection چه فرقی دارد؟

Push protection سر push می‌ایستد؛ secret اصلاً نباید به مخزن برسد. این قانون لایه دوم است، سر **Pull Request**.

مواردی که push protection نمی‌گیرد یا عمداً برایشان خاموش است — مثلاً الگوی generic — این‌جا می‌توانند جلوی merge را بگیرند. یعنی می‌شود push protection را برای generic شل گذاشت و ruleset را سخت، بدون این‌که developer هر push را به دیوار بزند.

این جایگزین push protection نیست؛ مکمل است.

---

## همان هفته: cache-mode برای Actions
...
این دو خبر یک تم دارند: کنترل نقطه ورود به main، یکی برای secret در کد، یکی برای مسموم کردن cache در CI.
...
اگر سازمان GHAS/Secret Protection دارید و هنوز فقط به reminder اکتفا کرده‌اید، این ruleset همان گیت سیاست است.

---

## چه کار کنید؟

- روی شاخه‌های محافظت‌شده ruleset بسازید و «Require secret scanning alerts are resolved» را روشن کنید.
- تصمیم بگیرید generic/custom هم بلاک شوند یا فقط provider.
- Bypass را محدود نگه دارید.
- در Actions، `cache-mode` را صریح کنید؛ برای `pull_request_target` پیش‌فرض read را بی‌دلیل write نکنید.
